...
export default function CustomFilter({ title, options }: CustomFilterProps) {
    ...
    const handleUpdateParams = (e: { title: string; value: string }) => {
        ...
        changeSearch(title, e.value.toLowerCase());
    };

    const changeSearch = (title: string, value: string) => {
        if (title === "fuel") {
            dispatch({ type: "SET_FUEL", payload: value })
        } else if (title === "year") {
            dispatch({ type: "SET_YEAR", payload: parseInt(value) })
        }
    }
